...
import { fetListComments } from "../Detail-Job/slide-comments"; // Redux action để lấy danh sách bình luận
import PostJobForm from "./thue";

function DetailJobPage() {
  const dispatch: AppDispatch = useDispatch();
  const { id } = useParams<{ id: string }>(); // Lấy ID từ URL
  const { data: job } = useSelector((state: RootState) => state.detailJobReducer);
  ...
  useEffect(() => {
    fetchComments(); // Lấy danh sách bình luận khi component được mount
  }, [id]);

  // Hàm callback để làm mới danh sách bình luận
  const handleCommentAdded = () => {
    fetchComments(); // Gọi lại API lấy danh sách bình luận
  };
        ...
        <div className="w-4/5">
          <div>
            <div className="p-4 ">
              ...
              {/* Truyền callback handleCommentAdded để làm mới comments sau khi thêm bình luận */}
